...
2. **Web Hosting & DNS:** Successfully load the custom website using the server's IP Address (`192.168.1.251`), the reverse proxy IP (`192.168.1.252`), and the configured Domain Name (`www.m2g10.istad` or `m2g10.istad`).
3. **Reverse Proxy:** Successfully load the same website by browsing to the DNS Server's IP (`192.168.1.252`), confirming Nginx is proxying traffic to the Apache backend.
